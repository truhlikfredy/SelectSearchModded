...
function goto(e){
	...
	chrome.extension.sendMessage({options: "goto", dest:_dest}, function(response) {});
	e.preventDefault();
    e.stopPropagation();
	return false;
	}
...
function gotofront(e){
    _dest = e.srcElement.parentElement.getAttribute('href');
    chrome.extension.sendMessage({options: "gotofront", dest:_dest}, function(response) {});
    e.preventDefault();
    e.stopPropagation();
    return false;
}	
...
var searchSelectBaloon = {
	...
	init : function(textbrut,text,e) {
		...
		if (document.getElementById("selectsearchcpboard")) {
		    chrome.extension.sendMessage({options: "setcp", text:textbrut}, function(response) {});
	...
	cpToClipboard : function() {
        chrome.extension.sendMessage({options: "toClipBoard"}, function(response) {});
        //searchSelectBaloon.remove()
	...
	links : function(text,textbrut) {
		...
		if (searchSelectBaloon.openIn == '_back' ) {
			...
			for(var i = 0; i < nbFils; i++){
			    if (fils[i].id != 'selectsearchcpboard') {
  				    fils[i].addEventListener("click", function(e) {goto(e);}, false);
  				}
			}		
		}
		...
		if (searchSelectBaloon.openIn == '_backf' ) {
			...
			for(var i = 0; i < nbFils; i++){
			    if (fils[i].id != 'selectsearchcpboard') {
  				    fils[i].addEventListener("click", function(e) {gotofront(e);}, false);
  				    fils[i].addEventListener("contextmenu", function(e) {goto(e);}, false);
  				}
			}		
		}
		...
		if (searchSelectBaloon.openIn == '_backfo' ) {
			...
			for(var i = 0; i < nbFils; i++){
			    if (fils[i].id != 'selectsearchcpboard') {
  				    fils[i].addEventListener("click", function(e) {goto(e);}, false);
  				    fils[i].addEventListener("contextmenu", function(e) {gotofront(e);}, false);
  				}
			}		
		}
...
//---------------------------
chrome.extension.sendMessage({options: "size"}, function(response) {
  searchSelectBaloon.size  = response.size;
  searchSelectBaloon.height  = response.height;
  //console.log(response.height);
});

chrome.extension.sendMessage({options: "times"}, function(response) {
  searchSelectBaloon.fadeTime  = response.fadeTime;
  searchSelectBaloon.removeTime  = response.removeTime;
  //console.log(response.height);
});

chrome.extension.sendMessage({options: "transparency"}, function(response) {
...
chrome.extension.sendMessage({options: "openIn"}, function(response) {
  searchSelectBaloon.openIn  = response.openIn;
});
chrome.extension.sendMessage({options: "links"}, function(response) {
